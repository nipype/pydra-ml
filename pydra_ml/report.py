...
def performance_table(df, output_dir, round_decimals = 2):
	...
	for idx, metric in enumerate(df.metric.unique()):
		...
		df_metric_data_clean.loc[len(df_metric_data_clean)] = df_metric_data_clean_median
		df_metric_data_clean.index = list(df_metric_data_clean.index[:-1]) + ['median']
		df_metric_data_clean.to_csv(os.path.join(output_dir, f'test-performance-table_{metric}_all-splits_{timestamp}.csv'))
		...
		for clf in classifier_names:
			...
			ci_lower = round(np.percentile(df_metric_data_clean[clf].values,2.5),round_decimals ) # 95% confidence interval
			ci_upper = round(np.percentile(df_metric_data_clean[clf].values,97.5),round_decimals )
			if 'null' in df_metric.type.unique():
				null_median = round(df_metric_null_clean_median[clf],2)
				df_summary.loc[0,clf] = f'{data_median} [{ci_lower}–{ci_upper}; {null_median}]'
			else:
				df_summary.loc[0,clf] = f'{data_median} [{ci_lower}–{ci_upper}]'

		df_summary.to_csv(os.path.join(output_dir, f'test-performance-table_{metric}_with-95ci_{timestamp}.csv'))
	return
...
def plot_summary(summary, output_dir=None, filename="shap_plot", plot_top_n_shap=16):
	...
	hm = sns.heatmap(
		summary.round(3), annot=True, xticklabels=True, yticklabels=True, cbar=False, square=True,annot_kws={"size": 12}
	)
...
def shaps_to_summary(shaps_n_splits,feature_names=None,output_dir=None,filename="shap_summary",plot_top_n_shap=16):
...
def gen_report(
	...
	for name, subdf in df.groupby("metric"):
		...
		if "score" in name:
			...
			ax = sns.heatmap(
				pvalues<=0.05, #ToDo: When effects has been implemented, set this to: effects
				annot= pvalues.round(3), #ToDo: When effects has been implemented, set this to: np.fix(-np.log10(pvalues))
				yticklabels=order,
				xticklabels=order,
