...
def test_sharp_test_parameter_recovery():
    ...
    # from the known miscalibration region -- see the module docstring),
...
def test_sharp_test_calibration_and_power_at_moderate_correlation():
    # At a correlation away from the known miscalibration region (see the
    # module docstring), false positive control and power both behave as
    # expected: FPR near nominal alpha, and high power for a clearly real
    # difference.
    ...
    false_positives = sum(
        sharp_test(
            *_simulate_sharp_data(J, 0.0, sigma2, rho, rng), experimental=True
        ).p_value
        < 0.05
    ...
    true_positives = sum(
        sharp_test(
            *_simulate_sharp_data(J, 2.0, sigma2, rho, rng), experimental=True
        ).p_value
        < 0.05
        for _ in range(n_sims)
    )
...
def test_sharp_test_not_anticonservative_at_or_below_zero_correlation():
    # At rho <= 0 (the floored point and below), false positive control
    # must never be violated in the unsafe direction (FPR >> alpha).
    rng = np.random.RandomState(2)
    n_sims = 300
    for J, rho in [(10, 0.0), (30, 0.0), (60, 0.0)]:
        false_positives = sum(
            sharp_test(
                *_simulate_sharp_data(J, 0.0, 1.0, rho, rng), experimental=True
            ).p_value
            < 0.05
            for _ in range(n_sims)
        )
...
def test_sharp_test_known_anticonservative_region():
    # Documents (as a regression benchmark, not a safety guarantee -- see
    # the prominent module docstring caveat) the known-bad miscalibration
    # region: small positive true correlation currently inflates the false
    # positive rate well above nominal. If this ever comes back near
    # nominal, that's good news -- update this test (and the caveat) to
    # match; if it gets worse, this should catch it.
    ...
    assert 0.1 < fpr < 0.35, f"FPR={fpr} outside the previously-measured bad range"
