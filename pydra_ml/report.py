...
def gen_report(results, prefix):
    ...
    for val in results:
        ...
        for aucval in auc:
            df = df.append({'AUC score': aucval if aucval is not None else np.nan,
                            'Classifier': name,
                            'type': 'null' if permute else 'data'}, ignore_index=True)
    sns.set(style="whitegrid", palette="pastel", color_codes=True)
    sns.set_context('talk')
    plt.figure(figsize=(12, 6))
    ax = sns.violinplot(x="Classifier", y="AUC score", hue="type", data=df, split=True,
                        inner="quartile", hue_order=['data', 'null'],
